...
export const changeSubmissionStatus = async (submissionId, status) => {
  const { data, error } = await supabase
    .from("submissions")
    .update({ requires_approval: false, is_approved: status })
    .eq("id", submissionId);
...
export const updateTeamScore = async (teamData) => {
  const { data: team, error: fetchError } = await getTeamScore(teamData.team_id)
  if (fetchError) throw new Error(fetchError.message);

  // Increment the points
  const newScore = team.score + teamData.points;
...
// TODO: Remove photopaths table completely
...
export const uploadSubmissionAndStoreInDB = async (huntId, teamId, taskId, submission) => {
  const id = uuidv4()
  const { data, error } = await supabase
    .storage
    .from("submissions")
...
export const updateUserHuntAndTeamId = async ({ huntId, teamId, userId }) => {
  const { error } = await supabase
    .from('profiles')
    .update({ huntId: huntId, teamId: teamId })
    .eq('id', userId)
}